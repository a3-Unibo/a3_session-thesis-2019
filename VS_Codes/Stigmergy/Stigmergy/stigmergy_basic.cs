...
public class Script_Instance : GH_ScriptInstance
{
    ...
    /// </summary>
    private void RunScript(bool reset, bool go, bool debug, Mesh M, List<Point3d> P, double sR, double sI, double eR, ref object points, ref object vectors, ref object tr, ref object cols, ref object neigh, ref object neighCol, ref object outMCol)
    {
        // <Custom code> 
        // ............................................................................
        // Stigmergy - basics
        ...
        if (reset || PS == null)
        {
            ...
            // populate scalar field from mesh
        ...
        points = pts;
        vectors = vecs;
        //neighbour points and colors (debug mode)
        if (debug)
        {
    ...
    public class ParticleSystem
    {
        ...
        public ParticleSystem(List<Point3d> positions, Mesh M)
        {
            ...
            for (int i = 0; i < positions.Count; i++)
                Particles.Add(new Particle(this, positions[i], RandomVector(i) * 1.5));
        ...
        public void UpdateAngVis()
        {
            foreach (Particle p in Particles)
            {
                // Seek brightest point direction
                p.SeekVisAng(seekIntensity);
    ...
    public class Particle
    {
        ...
        public void SeekVisAng(double seekIntensity)
        {
            acc = Vector3d.Zero;
            Vector3d rotAxis = MeshPoints[MeshPoints.ClosestPoint(pos)].Normal;
            Vector3d futPos = vel * pSystem.futPosMult;

            Vector3d futPosL = new Vector3d(futPos);
            Vector3d futPosR = new Vector3d(futPos);
            futPosL.Rotate(visAng * 0.5, rotAxis);
            futPosR.Rotate(visAng * -0.5, rotAxis);
            // find Left value
            int pL, pR;
            double briL, briR;
            pL = MeshPoints.ClosestPoint(pos + futPosL);
            pR = MeshPoints.ClosestPoint(pos + futPosR);
            briL = scalarField[pL];
            briR = scalarField[pR];
            ...
            if (briL - briR < 0.01)
            {
                ...
                desired = briL > briR ? futPosL : futPosR;
